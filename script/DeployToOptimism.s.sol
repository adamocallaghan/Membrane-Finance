...
contract DeployToOptimism is Script {
    function run() external {
        ...
        StableEngine optimismOapp = new StableEngine(vm.envAddress(OPTIMISM_LZ_ENDPOINT));
        console2.log("StableEngine Address: ", address(optimismOapp));

        // deploy StableCoin OFT contract
        StableCoin optimismOft =
            new StableCoin("Membrane USD", "memUSD", vm.envAddress(OPTIMISM_LZ_ENDPOINT), address(optimismOapp));
        console2.log("OFT Address: ", address(optimismOft));

        // deploy NFTMock
        NFTMock optimismNft = new NFTMock();
